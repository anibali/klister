...
object Main extends App {
  ...
  val joined = rdds(0).inequalityJoin(rdds(1), Comparison.Less, nReducers)
  //val joined = rdds(0).cartesian(rdds(1)).filter(x => x._1._1 < x._2._1)
  //val joined = rdds(0).thetaJoin(rdds(1), _ < _, nReducers)
